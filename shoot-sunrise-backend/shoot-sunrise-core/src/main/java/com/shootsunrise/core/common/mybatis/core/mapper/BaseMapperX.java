package com.shootsunrise.core.common.mybatis.core.mapper;

import cn.hutool.core.collection.CollUtil;
import com.shootsunrise.common.api.request.PageRequest;
import com.shootsunrise.common.api.response.PageResult;
import com.shootsunrise.core.common.mybatis.core.util.JdbcUtils;
import com.shootsunrise.core.common.mybatis.core.util.MyBatisUtils;
...
public interface BaseMapperX<T> extends MPJBaseMapper<T> {
    ...
    default PageResult<T> selectPage(PageRequest pageRequest, @Param("ew") Wrapper<T> queryWrapper) {
        ...
        // MyBatis Plus 查询
        IPage<T> mpPage = MyBatisUtils.buildPage(pageRequest);
        selectPage(mpPage, queryWrapper);
        // 转换返回
        return new PageResult<>(mpPage.getRecords(), mpPage.getTotal());
    }

    default <D> PageResult<D> selectJoinPage(PageRequest pageRequest, Class<D> clazz, MPJLambdaWrapper<T> lambdaWrapper) {
        // 特殊：不分页，直接查询全部
        if (PageRequest.PAGE_SIZE_NONE.equals(pageRequest.getPageSize())) {
        ...
        IPage<D> mpPage = MyBatisUtils.buildPage(pageRequest);
        mpPage = selectJoinPage(mpPage, clazz, lambdaWrapper);
        // 转换返回
        return new PageResult<>(mpPage.getRecords(), mpPage.getTotal());
    }

    default <DTO> PageResult<DTO> selectJoinPage(PageRequest pageRequest, Class<DTO> resultTypeClass, MPJBaseJoin<T> joinQueryWrapper) {
        IPage<DTO> mpPage = MyBatisUtils.buildPage(pageRequest);
